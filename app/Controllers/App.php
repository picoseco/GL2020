    ...
    */
    public function primarymenu() {
      $args = array(
        ...
        'menu_class'        => 'navbar-nav ml-auto nav',
    ...
    static function header_pag_corto($size="large",$post_id="post") {
      global $post;
    ...
    static function card_bg($size="large") {
      global $post;
      $title_id = $post->ID;
    ...
    static function card_bg_post($size="large",$page_slug="post") {
      global $post;
      $title_id = $post->ID;
    ...
    static function data_representante($child_post) {
      $html = '<div class="lugar mx-4">';
      $html .= '<h6>'.get_the_title($child_post).'</h6>';
      $html .= '<address>';
      $location = get_field('direccion',$child_post);
      if ($location) $html .= '<p><i class="fas fa-map-marked-alt"></i> '.$location['address'].'</p>';
      if (get_field( "direccion_xtra", $child_post)) $html .= '<p>'.get_field( "direccion_xtra", $child_post).'</p>';
      if (get_field( "email", $child_post)) $html .= '<p><i class="fas fa-envelope"></i> <a href="mailto:'.get_field( "email", $child_post).'">'.get_field( "email", $child_post).'</a></p>';
      if (get_field( "telefono", $child_post)) $html .= '<p><i class="fas fa-phone"></i> '.get_field( "telefono", $child_post);
      // if (get_field( "telefono", $child_post)) $html .= '<p><i class="fas fa-phone"></i> <a href="tel:'.get_field( "telefono", $child_post).'">'.get_field( "telefono", $child_post).'</a>';
      if (get_field( "telefono_alt", $child_post)) $html .= ' / '.get_field( "telefono_alt", $child_post);
      // if (get_field( "telefono_alt", $child_post)) $html .= ' / <a href="tel:'.get_field( "telefono_alt", $child_post).'">'.get_field( "telefono_alt", $child_post).'</a>';
      if (get_field( "telefono", $child_post)) $html .= '</p>';
      if (get_field( "web", $child_post)) $html .= '<p><i class="fas fa-atlas"></i> <a href="'.get_field( "web", $child_post).'" target="_blank">'.get_field( "web", $child_post).'</a></p>';
      if (get_field( "facebook", $child_post)) $html .= '<p class="text-truncate"><i class="fab fa-facebook-square"></i> <a href="'.get_field( "facebook", $child_post).'" target="_blank">'.get_field( "facebook", $child_post).'</a></p>';
      if (get_field( "instagram", $child_post)) $html .= '<p class="text-truncate"><i class="fab fa-instagram"></i> <a href="'.get_field( "instagram", $child_post).'" target="_blank">'.get_field( "instagram", $child_post).'</a></p>';
      $html .= '</address>';
      $html .= '</div>';
      return $html;
    }
